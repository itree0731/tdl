...
func NewUpload() *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.NoAutoThumb, "no-auto-thumb", false, "do not generate JPEG thumbnails for videos without a .thumb sidecar")
	cmd.Flags().StringVar(&opts.CoverMode, "cover-mode", "", "video cover mode: video-cover (default), thumbnail, or off")
	cmd.Flags().StringVar(&opts.CoverAt, "cover-at", "auto", "video cover frame: auto or a duration such as 12s")
	_ = cmd.Flags().MarkDeprecated("no-auto-thumb", "use --cover-mode off")
